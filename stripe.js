...
const Stripe_Key = "sk_test_51HwB2eJWOHWcUEhYDmAZdFrg5Hlu7ImRDdLyFJXGIasOMY3WBhAnfvyNevvtIVpkfSLW2boZw5zY4KGYE6OwxTc400cfLMI6W7";
const stripe = require("stripe")(Stripe_Key);
const customerId = "cus_L0K65AVYcpEO3L";
...
router.post("/createSubscription", async (req, res) => {
  try {
    const session = await stripe.subscriptions.create({
        ...
        items: [
          {
            price: "price_1KKJmWJWOHWcUEhYPBpcGIed"
          }
        ]
    });

    if (session.status === "active") {
        return res.status(200).send({ Success: session });
      } else {
        return res
          .status(400)
          .send({ Error: "Please try again later for payment" });
